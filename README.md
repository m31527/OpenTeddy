...
Runs on your machine with Ollama. Escalates to a cloud model only when it
actually gets stuck. Turns the work you repeat into reusable skills.

<sub>local AI agent · autonomous work · self-growing skills · hybrid escalation</sub>
...
## Your first finished task

Three commands. The destination is a finished artifact, not an installed program.

```bash
curl -fsSL https://openteddy.net/install | bash        # Python 3.10+ · Ollama recommended (pulls two small models)
cd ~/OpenTeddy && ./run.sh --open                       # dashboard at http://localhost:8000
./openteddy run "Analyze ~/OpenTeddy/examples/sales_sample.csv: monthly revenue trend, top 3 products, one chart. Save as HTML."
```

The terminal shows the plan, each tool call, and the result; the report
lands in the session's `reports/` folder and appears in the dashboard with
a 📎 chip. `examples/sales_sample.csv` ships with the repo so this works
before you point it at your own data. Prefer the browser? Paste the same
sentence into the dashboard.

No cloud key is needed for this. Add one later and OpenTeddy starts
escalating hard failures and growing skills — see *Pick your path*.

## Three reasons to care

| | |
|---|---|
| **Local control** | Planning and execution run on your hardware. A `local_only` session never calls a cloud API — your data, your models, your machine. |
| **Lower recurring cost** | The default path is $0 per token. A cloud model is called only when a step genuinely fails, and the Usage tab shows what the same work would have cost on GPT-4. |
| **Reusable skills** | Work you repeat gets turned into plain Python skills; the next time costs no reasoning at all. Skill code is generated by your configured cloud provider, then tested before it is registered. |

## Pick your path

| Path | What you need | What leaves your machine | What you get |
|---|---|---|---|
| **Try locally** | Python 3.10+, [Ollama](https://ollama.ai), 8 GB RAM for the default 2B models (16 GB comfortable). `OPENTEDDY_LLM_MODE=local` or Settings → *Local only*. | **Nothing.** | The full agent loop, tools, memory, dashboard, CLI, schedules, Telegram. No cloud escalation, no skill generation. |
| **Use hybrid models** *(default)* | The above + **one** cloud key — Anthropic, OpenAI, Gemini, Deepseek or OpenRouter — pasted in Settings → Cloud LLM Provider. | Only the subtasks that escalate after a hard local failure, plus skill-generation prompts. `local_only` sessions still send nothing. | + auto-escalation safety net, + self-growing skills. |
| **Developer / manual setup** | git, a venv, `.env` | as above | [Quick Start (manual)](#quick-start-manual), [Task API](#task-api-headless-runtime), [CLI](#cli). |
| **Desktop app (macOS)** | the `.dmg` above | as above | An onboarding wizard installs Ollama and pulls models for your hardware tier. |

## Three proof workflows

Each one is a real task ending in a real artifact. Try them in this order.

**1. Report from data** — analytic mode

```bash
./openteddy run "Analyze ~/OpenTeddy/examples/sales_sample.csv: monthly revenue trend, top 3 products, one chart. Save as HTML."
```
`csv_describe` → `python_exec` → `render_chart_report` → `reports/<name>.html` with a Chart.js chart and a markdown summary. A deliverable judge confirms the file is the report, not a description of one.

**2. A coding task** — code mode

```bash
./openteddy run "Create a Python CLI that converts a CSV file to JSON (with a --pretty flag) and a pytest test for it. Run the tests." --mode code
```
Files are written into the session workspace, the tests run in the shell, and the summary quotes the test output. High-risk commands pause for a `y/N` in your terminal.

**3. Repeated work becomes a skill** — hybrid path

Run the report task three times against different files. OpenTeddy spots the recurring pattern (ChromaDB similarity, default 3 repeats at 0.75), synthesises a skill from it with your cloud provider, tests it, and registers it:

```bash
./openteddy skill list                       # the new skill appears as TESTING, graduates to ACTIVE
./openteddy skill run <name> --input '{"path": "~/data/q3.csv"}'   # no planner, no reasoning, just the skill
```
...
```bash
cp .env.example .env
# Local-only: nothing else is required. OPENTEDDY_LLM_MODE=local guarantees no cloud calls.
# Hybrid (default): add ONE cloud key, e.g. ANTHROPIC_API_KEY=…  — or paste it later in
#                   Settings → Cloud LLM Provider. It is used only for escalation + skill generation.
```
